...
Original file is located at
    https://colab.research.google.com/drive/13sn73fDyyMY9idCogkzPht_zcsuvVjHA
...
"""# **Ablation Study 2 - PII Threshold**
...
# ============================================================================
# ABLATION STUDY 2: PII Frequency Threshold
# Location: New cell after k ablation
...
print("=" * 80)
print("ABLATION STUDY 2: Impact of PII Frequency Threshold")
print("=" * 80)

# Configuration
freq_thresholds = [2, 5, 10, 20, 30]
...
print(f"\nTesting frequency thresholds: {freq_thresholds}")
print(f"Samples per threshold: {num_canaries_ablation} canaries × {num_samples_ablation} alternatives")
print("=" * 80)

# Need to reload pii_frequencies from earlier
if 'pii_frequencies' not in globals():
    print("\n⚠️ WARNING: pii_frequencies not found. Using existing training_piis.")
    print("   Rerun the PII extraction cell if you want to test different thresholds.")
    # Create dummy frequencies from existing PIIs
    pii_frequencies = {}
    for pii_type in training_piis:
        pii_frequencies[pii_type] = Counter({pii: 5 for pii in training_piis[pii_type]})
...
for threshold in freq_thresholds:
    ...
    for pii_type in ['email', 'phone', 'ssn']:
        if pii_type in pii_frequencies:
            counter = pii_frequencies[pii_type]
            high_freq = [pii for pii, count in counter.items() if count >= threshold]
            filtered_piis[pii_type] = high_freq

            print(f"\n  {pii_type.upper()}:")
            print(f"    PIIs at freq >= {threshold}: {len(high_freq)}")

            if len(high_freq) > 0:
                sample_pii = high_freq[0]
                freq = counter[sample_pii]
                print(f"    Example: '{sample_pii}' appears {freq} times")

    # Check if we have enough PIIs
    available_types = []
    for ctype in ['email', 'phone']:
        if len(filtered_piis[ctype]) >= 10:
            available_types.append(ctype)
        else:
            print(f"  ⚠️ Skipping {ctype}: only {len(filtered_piis[ctype])} PIIs (need 10+)")

    if not available_types:
        print(f"  ❌ No PII types have enough samples at threshold {threshold}. Skipping.")
        continue

    print(f"\n  Testing types: {available_types}")

    # Run experiment with filtered PIIs
    results_freq = {
        'baseline': {},
        'rag': {},
        'threshold': threshold,
        'pii_counts': {k: len(v) for k, v in filtered_piis.items()}
    }

    for canary_type in available_types:
        print(f"\n  Testing {canary_type}...")

        # Generate canaries from filtered PIIs
        from eval.dea.canary_experiments import CanaryGenerator
        generator = CanaryGenerator(seed=42, training_piis=filtered_piis)
        canaries = []
        ...
        results_freq['baseline'][canary_type] = {
            'mean': np.mean(baseline_exposures),
            'std': np.std(baseline_exposures),
            'exposures': baseline_exposures
        }
        ...
        # Calculate reduction
        baseline_mean = results_freq['baseline'][canary_type]['mean']
        rag_mean = results_freq['rag'][canary_type]['mean']
        reduction = (baseline_mean - rag_mean) / baseline_mean * 100

        print(f"    Baseline: {baseline_mean:.2f} ± {results_freq['baseline'][canary_type]['std']:.2f}")
        print(f"    RAG:      {rag_mean:.2f} ± {results_freq['rag'][canary_type]['std']:.2f}")
        print(f"    Reduction: {reduction:.1f}%")

    # Store results
    ablation_freq_results['results'].append(results_freq)

    # Save checkpoint
    with open(f'ablation_freq_{threshold}_checkpoint.json', 'w') as f:
        json.dump(results_freq, f, indent=2)

    print(f"\n  ✅ Checkpoint saved: ablation_freq_{threshold}_checkpoint.json")

# Save final results
with open('ablation_freq_final.json', 'w') as f:
    json.dump(ablation_freq_results, f, indent=2)

print("\n" + "=" * 80)
print("✅ ABLATION STUDY 2 COMPLETE")
print("=" * 80)

# ============================================================================
# VISUALIZATION: Frequency Threshold Ablation Results
# ============================================================================
...
# Prepare data
thresholds_tested = []
baseline_by_thresh = {'email': [], 'phone': []}
rag_by_thresh = {'email': [], 'phone': []}
reduction_by_thresh = {'email': [], 'phone': []}
pii_counts = {'email': [], 'phone': []}

for result in ablation_freq_results['results']:
    thresholds_tested.append(result['threshold'])
    for ctype in ['email', 'phone']:
        if ctype in result['baseline']:
            baseline_mean = result['baseline'][ctype]['mean']
            rag_mean = result['rag'][ctype]['mean']
            baseline_by_thresh[ctype].append(baseline_mean)
            rag_by_thresh[ctype].append(rag_mean)
            reduction_by_thresh[ctype].append((baseline_mean - rag_mean) / baseline_mean * 100)
            pii_counts[ctype].append(result['pii_counts'].get(ctype, 0))
        else:
            baseline_by_thresh[ctype].append(None)
            rag_by_thresh[ctype].append(None)
            reduction_by_thresh[ctype].append(None)
            pii_counts[ctype].append(0)

# Plot 1: Baseline Exposure vs Frequency Threshold
ax1 = axes[0]
for ctype in ['email', 'phone']:
    valid_data = [(t, b) for t, b in zip(thresholds_tested, baseline_by_thresh[ctype]) if b is not None]
    if valid_data:
        t_vals, b_vals = zip(*valid_data)
        ax1.plot(t_vals, b_vals, marker='o', linewidth=2.5,
                markersize=8, label=f'{ctype.upper()} Baseline', alpha=0.8)
ax1.set_xlabel('PII Frequency Threshold', fontsize=11, fontweight='bold')
ax1.set_ylabel('Baseline Exposure', fontsize=11, fontweight='bold')
ax1.set_title('(A) Baseline Exposure vs PII Frequency', fontsize=12, fontweight='bold')
ax1.legend()
ax1.grid(alpha=0.3, linestyle='--')
ax1.set_xticks(thresholds_tested)

# Plot 2: RAG Exposure vs Frequency Threshold
ax2 = axes[1]
colors = {'email': '#e74c3c', 'phone': '#9b59b6'}
for ctype in ['email', 'phone']:
    valid_data = [(t, r) for t, r in zip(thresholds_tested, rag_by_thresh[ctype]) if r is not None]
    if valid_data:
        t_vals, r_vals = zip(*valid_data)
        color = colors.get(ctype, '#e74c3c')
        ax2.plot(t_vals, r_vals, marker='s', linewidth=2.5,
                markersize=8, label=f'{ctype.upper()} RAG', alpha=0.8, color=color)
ax2.set_xlabel('PII Frequency Threshold', fontsize=11, fontweight='bold')
ax2.set_ylabel('RAG Exposure', fontsize=11, fontweight='bold')
ax2.set_title('(B) RAG Exposure vs PII Frequency', fontsize=12, fontweight='bold')
ax2.legend()
ax2.grid(alpha=0.3, linestyle='--')
ax2.set_xticks(thresholds_tested)

# Plot 3: Reduction % vs Frequency Threshold
ax3 = axes[2]
colors_reduction = {'email': '#27ae60', 'phone': '#16a085'}
for ctype in ['email', 'phone']:
    valid_data = [(t, r) for t, r in zip(thresholds_tested, reduction_by_thresh[ctype]) if r is not None]
    if valid_data:
        t_vals, red_vals = zip(*valid_data)
        color = colors_reduction.get(ctype, '#27ae60')
        ax3.plot(t_vals, red_vals, marker='D', linewidth=2.5,
                markersize=8, label=f'{ctype.upper()}', alpha=0.8, color=color)
        # Add value labels
        for t, red in zip(t_vals, red_vals):
            ax3.text(t, red + 2, f'{red:.1f}%', ha='center', fontsize=8, fontweight='bold')

ax3.axhline(y=60, color='red', linestyle='--', linewidth=2, alpha=0.6, label='60% Target')
ax3.set_xlabel('PII Frequency Threshold', fontsize=11, fontweight='bold')
ax3.set_ylabel('Privacy Leakage Reduction (%)', fontsize=11, fontweight='bold')
ax3.set_title('(C) Reduction vs PII Frequency', fontsize=12, fontweight='bold')
ax3.legend()
ax3.grid(alpha=0.3, linestyle='--')
ax3.set_xticks(thresholds_tested)

# Plot 4: Number of Available PIIs vs Threshold
ax4 = axes[3]
colors_count = {'email': '#3498db', 'phone': '#f39c12'}
for ctype in ['email', 'phone']:
    valid_data = [(t, c) for t, c in zip(thresholds_tested, pii_counts[ctype]) if c > 0]
    if valid_data:
        t_vals, c_vals = zip(*valid_data)
        color = colors_count.get(ctype, '#3498db')
        ax4.plot(t_vals, c_vals, marker='o', linewidth=2.5,
                markersize=8, label=f'{ctype.upper()}', alpha=0.8, color=color)
        # Add value labels
        for t, c in zip(t_vals, c_vals):
            ax4.text(t, c + 5, f'{c}', ha='center', fontsize=9, fontweight='bold')

ax4.axhline(y=10, color='red', linestyle='--', linewidth=2, alpha=0.6, label='Minimum (10)')
ax4.set_xlabel('PII Frequency Threshold', fontsize=11, fontweight='bold')
ax4.set_ylabel('Number of Available PIIs', fontsize=11, fontweight='bold')
ax4.set_title('(D) PII Availability vs Threshold', fontsize=12, fontweight='bold')
ax4.legend()
ax4.grid(alpha=0.3, linestyle='--')
ax4.set_xticks(thresholds_tested)
ax4.set_yscale('log')

plt.tight_layout()
plt.savefig('ablation_freq_results.png', dpi=300, bbox_inches='tight')
plt.show()

# Print summary table
print("\n" + "=" * 80)
print("ABLATION SUMMARY: Impact of PII Frequency Threshold")
print("=" * 80)
print(f"{'Thresh':<8} {'Email #':<10} {'Email Base':<12} {'Email RAG':<12} {'Email Red%':<12} "
      f"{'Phone #':<10} {'Phone Base':<12} {'Phone RAG':<12} {'Phone Red%':<12}")
print("-" * 80)

for i, thresh in enumerate(thresholds_tested):
    e_count = pii_counts['email'][i]
    e_base = baseline_by_thresh['email'][i] if baseline_by_thresh['email'][i] else 0
    e_rag = rag_by_thresh['email'][i] if rag_by_thresh['email'][i] else 0
    e_red = reduction_by_thresh['email'][i] if reduction_by_thresh['email'][i] else 0

    p_count = pii_counts['phone'][i]
    p_base = baseline_by_thresh['phone'][i] if baseline_by_thresh['phone'][i] else 0
    p_rag = rag_by_thresh['phone'][i] if rag_by_thresh['phone'][i] else 0
    p_red = reduction_by_thresh['phone'][i] if reduction_by_thresh['phone'][i] else 0

    print(f"{thresh:<8} {e_count:<10} {e_base:<12.2f} {e_rag:<12.2f} {e_red:<12.1f} "
          f"{p_count:<10} {p_base:<12.2f} {p_rag:<12.2f} {p_red:<12.1f}")

print("=" * 80)

# Key insights
print("\nKEY INSIGHTS:")
print(f"1. Threshold range tested: {min(thresholds_tested)} - {max(thresholds_tested)}")

# Find threshold with highest baseline (strongest memorization)
max_baseline_thresh = None
max_baseline_val = 0
for i, thresh in enumerate(thresholds_tested):
    avg_baseline = np.mean([baseline_by_thresh[ct][i] for ct in ['email', 'phone']
                           if baseline_by_thresh[ct][i] is not None])
    if avg_baseline > max_baseline_val:
        max_baseline_val = avg_baseline
        max_baseline_thresh = thresh

print(f"2. Strongest memorization at threshold: {max_baseline_thresh} (baseline exposure: {max_baseline_val:.2f})")

# Find optimal threshold (best reduction)
max_reduction_thresh = None
max_reduction_val = 0
for i, thresh in enumerate(thresholds_tested):
    avg_reduction = np.mean([reduction_by_thresh[ct][i] for ct in ['email', 'phone']
                            if reduction_by_thresh[ct][i] is not None])
    if avg_reduction > max_reduction_val:
        max_reduction_val = avg_reduction
        max_reduction_thresh = thresh

print(f"3. Best reduction at threshold: {max_reduction_thresh} ({max_reduction_val:.1f}% reduction)")

print("\n✅ Results saved to 'ablation_freq_final.json'")
print("✅ Plots saved to 'ablation_freq_results.png'")