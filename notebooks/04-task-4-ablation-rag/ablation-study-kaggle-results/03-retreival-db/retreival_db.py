...
Original file is located at
    https://colab.research.google.com/drive/10BWuYe3jU8YiURhy4TqYHeFRgDsTwDjN
...
"""The below cell will fail if you dont restart for a second time"""
...
if not os.path.exists(config_path):
    default_config = {
        ...
        'dea': {
            ...
            'canary_repetitions': [1, 5, 10, 20, 50], # reduce for faster experiment
            'num_canaries_per_type': 30, # Changing this from 10 to 50
...
"""### 3.2. Functions, Loading Dataset & Path Configurations"""

# Load Enron Data from The Pile (Correct Splits)
...
# Extract Real PIIs from Training Emails
...
print("\n1. Filtering for PIIs with frequency >= 5...")
...
for pii_type in ['email', 'phone', 'ssn']:
    if pii_type in pii_frequencies:
        ...
        high_freq_piis = [pii for pii, count in counter.items() if count >= 5]
...
"""### 3.3. Running Baseline Tests & Plotting"""
...
if not available_canary_types:
    ...
    baseline_results = run_dea_experiment(
        ...
        canary_repetitions=[1, 5, 10, 20, 50],  # Single repetition for speed
        num_canaries=30,
        output_dir='./results',
        seed=42,
...
### 4.1. Load Model & Dataset
"""

# Load GPT-Neo and Setup
...
# VERIFICATION: Test Retrieval Pipeline
# retreive context function, Compute exposure and perplexity functions
...
"""### 4.3. Loading Baseline results and Training PII"""
...
"""### 4.4. Running Baseline + Rag Test & Plotting"""
...
from canary_experiments import run_dea_experiment, CanaryGenerator
from tqdm.auto import tqdm

print("=" * 60)
print("RQ2 EXPERIMENT: RAG Privacy Leakage")
print("=" * 60)
...
if not available_types:
    ...
    results_fixed = run_rq2_experiment(
        ...
        num_canaries=100,  # Increased from 10 for better statistics
        num_samples=250,
        device=device,
...
bars = axes[1].bar(canary_types, reduction_percentages,
                   color='#2ecc71', alpha=0.8)

axes[1].axhline(y=60, color='red', linestyle='--',
                label='60% Target', linewidth=2)
...
axes[1].set_xlabel('Canary Type', fontsize=11)
axes[1].set_ylabel('Reduction (%)', fontsize=11)
axes[1].set_title('Privacy Leakage Reduction',
...
# ============================================================================
# ABLATION STUDY 4: Retrieval Database Size
# Location: New cell after context length ablation
# Runtime: ~90-120 minutes on T4 GPU (includes rebuilding databases)
# ============================================================================
...
print("=" * 80)
print("ABLATION STUDY 4: Impact of Retrieval Database Size")
print("=" * 80)

# Configuration
db_sizes = [1000, 2500, 5000, 10000, 25000]
num_canaries_ablation = 30
num_samples_ablation = 100
k_value = 2
...
available_types = ['email', 'phone'] if 'email' in training_piis and 'phone' in training_piis else ['email']

print(f"\nTesting database sizes: {db_sizes} documents")
print(f"Fixed k value: {k_value}")
print(f"Canary types: {available_types}")
print("=" * 80)

# Check if we have enough retrieval docs
if len(retrieval_docs) < max(db_sizes):
    print(f"\n⚠️ WARNING: Only {len(retrieval_docs)} retrieval docs available")
    print(f"   Maximum requested: {max(db_sizes)}")
    print(f"   Adjusting db_sizes to available data...")
    db_sizes = [size for size in db_sizes if size <= len(retrieval_docs)]
    print(f"   New db_sizes: {db_sizes}")

for db_size in db_sizes:
    print(f"\n{'='*80}")
    print(f"Testing Database Size = {db_size} documents")
    print(f"{'='*80}")

    # Rebuild ChromaDB with subset of documents
    print(f"\n  Building vector database with {db_size} documents...")

    # Create new collection for this size
    chroma_client_ablation = chromadb.Client()
    collection_name_ablation = f"wiki_retrieval_{db_size}"

    # Clear if exists
    try:
        chroma_client_ablation.delete_collection(collection_name_ablation)
    except:
        pass

    collection_ablation = chroma_client_ablation.create_collection(
        name=collection_name_ablation,
        metadata={"hnsw:space": "cosine"}
    )

    # Get subset of docs
    docs_subset = retrieval_docs[:db_size]

    # Embed and index in batches
    batch_size = 50
    for i in range(0, len(docs_subset), batch_size):
        batch = docs_subset[i:i+batch_size]
        embeddings = embed_model.encode(batch, convert_to_numpy=True)

        collection_ablation.add(
            embeddings=embeddings.tolist(),
            documents=batch,
            ids=[f"doc_{j}" for j in range(i, i+len(batch))]
        )
        ...
        if (i // batch_size) % 5 == 0:
            print(f"    Indexed {i+len(batch)}/{len(docs_subset)} documents", end='\r')

    print(f"    ✅ Vector database ready with {db_size} documents")

    # Modified exposure function to use ablation collection
    def compute_exposure_with_db(model, tokenizer, canary, canary_type,
                                num_samples=100, use_rag=False, k=2,
                                device='cuda', retrieval_seed=None,
                                collection_to_use=None, docs_to_use=None):
        """Compute exposure using specified collection"""
        from eval.dea.canary_experiments import CanaryGenerator
        import random

        if use_rag:
            # Retrieve contexts from specified collection
            query_text = canary
            query_embedding = embed_model.encode([query_text], convert_to_numpy=True)
            results = collection_to_use.query(
                query_embeddings=query_embedding.tolist(),
                n_results=k
            )
            contexts = results['documents'][0]
        ...
        for i in range(num_samples - 1):
            ...
            if use_rag:
                if retrieval_seed is not None:
                    random.seed(retrieval_seed + i + 1)
                random_ids = random.sample(range(len(docs_to_use)), k)
                alt_contexts = [docs_to_use[j] for j in random_ids]
                alt_context_text = " ".join(alt_contexts) + "\n\n"

                alt_perp = compute_perplexity(model, tokenizer, alt_canary, device,
                                             context_text=alt_context_text)
            else:
                alt_perp = compute_perplexity(model, tokenizer, alt_canary, device)

            perplexities.append(alt_perp)

        # Compute rank and exposure
        perplexities = np.array(perplexities)
        rank = np.sum(perplexities <= canary_perp)
        exposure = np.log2(num_samples) - np.log2(rank)
    ...
    # Run experiment with this database size
    results_db = {
        'baseline': {},
        'rag': {},
        'db_size': db_size
    ...
    for canary_type in available_types:
        print(f"\n  Testing {canary_type}...")
        ...
        generator = CanaryGenerator(seed=42, training_piis=training_piis)
        canaries = []
        for i in range(num_canaries_ablation):
            canary, secret = generator.generate_canary(canary_type)
            canaries.append((canary, secret))

        # Baseline (reuse if available)
        if canary_type in results_fixed['baseline']:
            main_exposures = results_fixed['baseline'][canary_type]['exposures']
            sampled_exposures = np.random.choice(main_exposures,
                                                size=min(num_canaries_ablation, len(main_exposures)),
                                                replace=False).tolist()
            results_db['baseline'][canary_type] = {
                'mean': np.mean(sampled_exposures),
                'std': np.std(sampled_exposures),
                'exposures': sampled_exposures
            }
        else:
            baseline_exposures = []
            for canary, secret in tqdm(canaries, desc=f"  {canary_type} baseline"):
                exp_base, _, _ = compute_exposure_with_db(
                    model, tokenizer, canary, canary_type,
                    num_samples=num_samples_ablation,
                    use_rag=False,
                    device=device,
                    collection_to_use=collection_ablation,
                    docs_to_use=docs_subset
                )
                baseline_exposures.append(exp_base)
        ...
        # RAG with current database size
        rag_exposures = []
        for i, (canary, secret) in enumerate(tqdm(canaries, desc=f"  {canary_type} RAG (db={db_size})")):
            exp_rag, _, _ = compute_exposure_with_db(
                model, tokenizer, canary, canary_type,
                num_samples=num_samples_ablation,
                use_rag=True,
                k=k_value,
        ...
        # Calculate reduction
        baseline_mean = results_db['baseline'][canary_type]['mean']
        rag_mean = results_db['rag'][canary_type]['mean']
        reduction = (baseline_mean - rag_mean) / baseline_mean * 100

        print(f"    Baseline: {baseline_mean:.2f} ± {results_db['baseline'][canary_type]['std']:.2f}")
        print(f"    RAG:      {rag_mean:.2f} ± {results_db['rag'][canary_type]['std']:.2f}")
        print(f"    Reduction: {reduction:.1f}%")

    # Store results
    ablation_db_results['results'].append(results_db)

    # Save checkpoint
    with open(f'ablation_db_{db_size}_checkpoint.json', 'w') as f:
        json.dump(results_db, f, indent=2)

    print(f"\n  ✅ Checkpoint saved: ablation_db_{db_size}_checkpoint.json")

# Save final results
with open('ablation_db_final.json', 'w') as f:
    json.dump(ablation_db_results, f, indent=2)

print("\n" + "=" * 80)
print("✅ ABLATION STUDY 4 COMPLETE")
print("=" * 80)

# ============================================================================
# VISUALIZATION: Database Size Ablation Results
# ============================================================================
...
# Prepare data
sizes = []
baseline_by_size = {ctype: [] for ctype in available_types}
rag_by_size = {ctype: [] for ctype in available_types}
reduction_by_size = {ctype: [] for ctype in available_types}

for result in ablation_db_results['results']:
    sizes.append(result['db_size'])
    for ctype in available_types:
        baseline_mean = result['baseline'][ctype]['mean']
        rag_mean = result['rag'][ctype]['mean']
        baseline_by_size[ctype].append(baseline_mean)
        rag_by_size[ctype].append(rag_mean)
        reduction_by_size[ctype].append((baseline_mean - rag_mean) / baseline_mean * 100)

# Plot 1: Baseline vs Database Size
ax1 = axes[0]
for ctype in available_types:
    ax1.plot(sizes, baseline_by_size[ctype], marker='o', linewidth=2,
            markersize=8, label=f'{ctype.upper()} Baseline', alpha=0.8)
ax1.set_xlabel('Database Size (documents)', fontsize=11, fontweight='bold')
ax1.set_ylabel('Baseline Exposure', fontsize=11, fontweight='bold')
ax1.set_title('(A) Baseline Exposure vs Database Size', fontsize=12, fontweight='bold')
ax1.legend()
ax1.grid(alpha=0.3, linestyle='--')
ax1.set_xscale('log')
ax1.set_xticks(sizes)
ax1.set_xticklabels([f'{s/1000:.1f}K' for s in sizes], rotation=0)

# Plot 2: RAG Exposure vs Database Size
ax2 = axes[1]
colors = {'email': '#e74c3c', 'phone': '#9b59b6'}
for ctype in available_types:
    color = colors.get(ctype, '#e74c3c')
    ax2.plot(sizes, rag_by_size[ctype], marker='s', linewidth=2.5,
            markersize=8, label=f'{ctype.upper()} RAG', alpha=0.8, color=color)
ax2.set_xlabel('Database Size (documents)', fontsize=11, fontweight='bold')
ax2.set_ylabel('RAG Exposure', fontsize=11, fontweight='bold')
ax2.set_title('(B) RAG Exposure vs Database Size', fontsize=12, fontweight='bold')
ax2.legend()
ax2.grid(alpha=0.3, linestyle='--')
ax2.set_xscale('log')
ax2.set_xticks(sizes)
ax2.set_xticklabels([f'{s/1000:.1f}K' for s in sizes], rotation=0)

# Plot 3: Reduction % vs Database Size
ax3 = axes[2]
colors_reduction = {'email': '#27ae60', 'phone': '#16a085'}
for ctype in available_types:
    color = colors_reduction.get(ctype, '#27ae60')
    ax3.plot(sizes, reduction_by_size[ctype], marker='D', linewidth=2.5,
            markersize=8, label=f'{ctype.upper()}', alpha=0.8, color=color)
    # Add value labels
    for i, (s, red) in enumerate(zip(sizes, reduction_by_size[ctype])):
        ax3.text(s, red + 2, f'{red:.1f}%', ha='center', fontsize=8, fontweight='bold')

ax3.axhline(y=60, color='red', linestyle='--', linewidth=2, alpha=0.6, label='60% Target')
ax3.set_xlabel('Database Size (documents)', fontsize=11, fontweight='bold')
ax3.set_ylabel('Privacy Leakage Reduction (%)', fontsize=11, fontweight='bold')
ax3.set_title('(C) Reduction vs Database Size', fontsize=12, fontweight='bold')
ax3.legend()
ax3.grid(alpha=0.3, linestyle='--')
ax3.set_xscale('log')
ax3.set_xticks(sizes)
ax3.set_xticklabels([f'{s/1000:.1f}K' for s in sizes], rotation=0)

plt.tight_layout()
plt.savefig('ablation_db_results.png', dpi=300, bbox_inches='tight')
plt.show()

# Print summary table
print("\n" + "=" * 80)
print("ABLATION SUMMARY: Impact of Database Size")
print("=" * 80)
print(f"{'DB Size':<12} {'Email Base':<15} {'Email RAG':<15} {'Email Red%':<15} "
      f"{'Phone Base':<15} {'Phone RAG':<15} {'Phone Red%':<15}")
print("-" * 80)

for i, size in enumerate(sizes):
    e_base = baseline_by_size['email'][i] if 'email' in available_types else 0
    e_rag = rag_by_size['email'][i] if 'email' in available_types else 0
    e_red = reduction_by_size['email'][i] if 'email' in available_types else 0

    p_base = baseline_by_size['phone'][i] if 'phone' in available_types else 0
    p_rag = rag_by_size['phone'][i] if 'phone' in available_types else 0
    p_red = reduction_by_size['phone'][i] if 'phone' in available_types else 0

    print(f"{size:<12} {e_base:<15.2f} {e_rag:<15.2f} {e_red:<15.1f} "
          f"{p_base:<15.2f} {p_rag:<15.2f} {p_red:<15.1f}")
...
# Find optimal size
optimal_idx = np.argmax([np.mean([reduction_by_size[ct][i] for ct in available_types])
                        for i in range(len(sizes))])
print(f"1. Optimal database size: {sizes[optimal_idx]:,} documents "
      f"({np.mean([reduction_by_size[ct][optimal_idx] for ct in available_types]):.1f}% reduction)")

# Check scaling behavior
if len(sizes) >= 3:
    # Compute improvements per 10x increase
    improvements = []
    for i in range(1, len(sizes)):
        size_ratio = sizes[i] / sizes[i-1]
        avg_red_prev = np.mean([reduction_by_size[ct][i-1] for ct in available_types])
        avg_red_curr = np.mean([reduction_by_size[ct][i] for ct in available_types])
        improvement = avg_red_curr - avg_red_prev
        improvements.append((size_ratio, improvement))

    print(f"2. Scaling behavior:")
    for i, (ratio, imp) in enumerate(improvements):
        print(f"   {sizes[i]:>6} → {sizes[i+1]:>6} docs ({ratio:>4.1f}x): {imp:>+5.1f}% improvement")

    # Check for logarithmic scaling
    avg_improvement = np.mean([imp for _, imp in improvements])
    if avg_improvement > 0:
        print(f"3. ✅ Larger databases improve reduction (avg +{avg_improvement:.1f}% per increase)")
    else:
        print(f"3. ⚠️ Larger databases reduce effectiveness (avg {avg_improvement:.1f}% per increase)")

# Cost-benefit analysis
smallest_reduction = np.mean([reduction_by_size[ct][0] for ct in available_types])
largest_reduction = np.mean([reduction_by_size[ct][-1] for ct in available_types])
size_increase = sizes[-1] / sizes[0]

print(f"\n4. Cost-Benefit Analysis:")
print(f"   Size increase: {size_increase:.0f}x ({sizes[0]:,} → {sizes[-1]:,} docs)")
print(f"   Reduction gain: {largest_reduction - smallest_reduction:.1f}% "
      f"({smallest_reduction:.1f}% → {largest_reduction:.1f}%)")
print(f"   Efficiency: {(largest_reduction - smallest_reduction) / np.log10(size_increase):.2f}% per 10x size increase")

print("\n✅ Results saved to 'ablation_db_final.json'")
print("✅ Plots saved to 'ablation_db_results.png'")