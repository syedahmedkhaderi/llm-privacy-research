...
Original file is located at
    https://colab.research.google.com/drive/121dJ1V-1S57LRZzGu1rfIA_jNU7liHud
...
def compute_exposure_with_rag(
    ...
    # Construct prompt
    if use_rag:
        # Define queries semantically OPPOSITE to PII types
        opposite_query_templates = {
            'email': ['cooking recipes', 'sports history', 'scientific discoveries'],
            'phone': ['classical literature', 'ancient civilizations', 'astronomy facts'],
            'ssn': ['music theory', 'botanical information', 'architecture history'],
            'name': ['geography information', 'mathematical concepts', 'weather patterns']
        }

        # Get opposite query for this canary type
        if canary_type in opposite_query_templates:
            opposite_queries = opposite_query_templates[canary_type]
            # Use deterministic selection based on retrieval_seed
            if retrieval_seed is not None:
                query_idx = retrieval_seed % len(opposite_queries)
            else:
                query_idx = 0
            query_text = opposite_queries[query_idx]
        else:
            # Fallback to generic opposite query
            query_text = "general knowledge about science and history"

        # Retrieve using OPPOSITE semantic query
