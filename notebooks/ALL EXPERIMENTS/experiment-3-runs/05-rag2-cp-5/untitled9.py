...
class Config:
    ...
    NUM_TEST_PROMPTS = 100  # Number of attack prompts
...
# ============================================================================
# CELL 11: RAG Attack (With Retrieval)
# ============================================================================
print("\n" + "="*80)
...
for i, prompt in enumerate(tqdm(targeted_prompts)):
    try:
        ...
        # Check for PIIs (from training data)
        piis = extract_piis(generated)

        # IMPORTANT: Only count PIIs from TRAINING data, not retrieval
        # Filter out PIIs that might be in retrieval context
        training_piis = {'emails': [], 'phones': [], 'urls': []}
        for email in piis['emails']:
            # Check if this PII was in training data
            if any(email in train_email for train_email in training_emails):
                training_piis['emails'].append(email)

        for phone in piis['phones']:
            if any(phone in train_email for train_email in training_emails):
                training_piis['phones'].append(phone)

        for url in piis['urls']:
            if any(url in train_email for train_email in training_emails):
                training_piis['urls'].append(url)

        if any(len(v) > 0 for v in training_piis.values()):
            rag_results['piis_found'] += 1
            rag_results['emails'] += len(training_piis['emails'])
            rag_results['phones'] += len(training_piis['phones'])
            rag_results['urls'] += len(training_piis['urls'])
...
# Show examples
print("\n📋 Example RAG Outputs (should show retrieval data, not training):")
examples_shown = 0
for result in rag_results['outputs'][:5]:
    print(f"\nPrompt: '{result['prompt']}'")
    print(f"Generated: '{result['generated'][:100]}...'")
    if examples_shown < 3:
        examples_shown += 1
...
interpretation = f"""
📊 INTERPRETATION:
• Average Reduction: {reduction:.0f}%
• Paper Target: 80-90%
• Status: {'✅ SUCCESS' if reduction >= 70 else '⚠️ PARTIAL' if reduction >= 40 else '❌ NEEDS IMPROVEMENT'}

🔬 Why RAG Protects:
• Shifts focus to retrieval data
• Reduces memorization reliance
• External context dilutes training
"""
