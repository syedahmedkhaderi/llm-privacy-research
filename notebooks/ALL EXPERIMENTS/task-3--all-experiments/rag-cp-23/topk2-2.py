...
Original file is located at
    https://colab.research.google.com/drive/19panaIvVqebmrsWfiaeUG-Dz1JwTGV--
"""
...
retrieval_docs = retrieval_docs[:50000] # Changed from 10,000.
print(f"✓ Loaded {len(retrieval_docs)} documents for retrieval")
print(f"Sample doc: {retrieval_docs[0][:200]}...")

# ============================================================================
# RE-EXTRACT TRAINING PIIs (AFTER RESTART)
# Critical: training_piis was lost during runtime restart
# ============================================================================

print("=" * 60)
print("RE-LOADING TRAINING DATA (POST-RESTART)")
print("=" * 60)

# Check if training_piis exists
try:
    if training_piis:
        print("✅ training_piis already in memory")
except NameError:
    print("⚠️  training_piis not found - re-extracting...")

    # Re-load Enron dataset
    from datasets import load_dataset
    import re
    from collections import Counter

    print("\n1. Loading pile_enron dataset...")
    enron_val = load_dataset('suolyer/pile_enron', split='validation')
    enron_test = load_dataset('suolyer/pile_enron', split='test')
    enron_pile = list(enron_val) + list(enron_test)
    ...
    print(f"   ✅ Re-loaded {len(training_emails)} emails")
    ...
    print("\n2. Re-extracting PIIs...")
    training_piis = {'email': [], 'phone': [], 'ssn': []}

    for email_text in training_emails:
        training_piis['email'].extend(extract_emails_from_text(email_text))
        training_piis['phone'].extend(extract_phones_from_text(email_text))

    # Deduplicate - keep PIIs appearing 2+ times
    for pii_type in training_piis:
        counter = Counter(training_piis[pii_type])
        training_piis[pii_type] = [pii for pii, count in counter.items() if count >= 2]

    print(f"   ✅ Re-extracted PIIs:")
    print(f"      Emails: {len(training_piis['email'])}")
    print(f"      Phones: {len(training_piis['phone'])}")

# Verify training_piis is valid
if not training_piis or all(len(training_piis[k]) == 0 for k in training_piis):
    print("\n❌ ERROR: No valid training PIIs!")
    print("Cannot proceed with high-exposure experiment")
else:
    print(f"\n✅ training_piis ready for RQ2 experiment")
    print("=" * 60)
...
# ============================================================================
# CELL 5 (FIXED) - VERIFICATION: Test Retrieval Pipeline
# CRITICAL: Run this before any long experiments
# ============================================================================

def retrieve_contexts_for_query(query, k=2, collection=None, embed_model=None):
    """
    Retrieve top-k RELEVANT documents using query embedding.
    Matches paper behavior (semantic retrieval, not random).
    """

    # ---- Safety checks (CRITICAL) ----
    if embed_model is None:
        raise ValueError("embed_model is None. Make sure embedding model is initialized.")
    if collection is None:
        raise ValueError("collection is None. Make sure Chroma collection is created.")
...
for query in test_queries:
    contexts = retrieve_contexts_for_query(
        query,
        k=2,
        collection=collection,        # EXPLICIT
...
def compute_exposure_with_rag_FIXED(
    model,
    tokenizer,
    canary,
    canary_type,
    ...
    k=2,
    device='cuda',
    collection=None,      # ADD THESE
    embed_model=None,     # ADD THESE
    training_piis=None
):
    """
    FIXED VERSION: Uses query-based retrieval, not random sampling
    """
    from eval.dea.canary_experiments import CanaryGenerator

    # Construct query-based retrieval context
    if use_rag:
        # CORRECT: Retrieve based on canary content (query-based)
        contexts = retrieve_contexts_for_query(canary, k, collection, embed_model)
        context_text = "Context: " + " ".join(contexts) + "\n\n"

        # Compute perplexity ONLY on canary, with relevant context prepended
        canary_perp = compute_perplexity(model, tokenizer, canary, device, context_text=context_text)
    else:
        # Baseline: no RAG
        canary_perp = compute_perplexity(model, tokenizer, canary, device)

    # Generate alternatives with QUERY-BASED retrieval
    ...
    for i in range(num_samples - 1):
        ...
        if use_rag:
            # CORRECT: Each alternative gets its OWN relevant context
            alt_contexts = retrieve_contexts_for_query(alt_canary, k, collection, embed_model)
...
# Update your experiment call:
def run_rq2_experiment_FIXED(
    model,
    tokenizer,
    canary_types=['email', 'phone'],
    num_canaries=10,
    num_samples=100,
    device='cuda',
    training_piis=None,
    baseline_results=None,
    collection=None,        # PASS THESE
    embed_model=None        # PASS THESE
):
    """Updated experiment with proper retrieval"""
    ...
    for canary_type in canary_types:
        ...
        # Compute RAG with PROPER retrieval
        rag_exposures = []
        for i in tqdm(range(num_canaries), desc=f"{canary_type} RAG"):
            canary, _ = generator.generate_canary(canary_type)

            exp_rag, _, _ = compute_exposure_with_rag_FIXED(
                model, tokenizer, canary, canary_type,
                num_samples=num_samples,
                use_rag=True,
                k=2,
                device=device,
                collection=collection,      # PASS THESE
                embed_model=embed_model,    # PASS THESE
        ...
        logger.info(f"  Baseline: {results['baseline'][canary_type]['mean']:.2f}")
        logger.info(f"  RAG: {results['rag'][canary_type]['mean']:.2f}")
        logger.info(f"  Reduction: {results['baseline'][canary_type]['mean'] - results['rag'][canary_type]['mean']:.2f}")
...
from eval.dea.canary_experiments import CanaryGenerator, compute_exposure
...
exp_baseline, rank_base, perp_base = compute_exposure(
...
exp_rag, rank_rag, perp_rag = compute_exposure_with_rag_FIXED(
    model, tokenizer, test_canary, 'email',
    num_samples=20,
    use_rag=True,
    k=2,  # Changed from 2 to 5
    device=device,
    collection=collection,      # ADD THESE
    embed_model=embed_model,     # ADD THESE
)
...
results = run_rq2_experiment_FIXED(
    model=model,
    tokenizer=tokenizer,
    ...
    device=device,
    training_piis=training_piis,  # PASS TRAINING DATA
    baseline_results=baseline_from_dea,  # PASS PRE-COMPUTED BASELINE
    collection=collection,       # ADD THIS
    embed_model=embed_model       # ADD THIS
)
