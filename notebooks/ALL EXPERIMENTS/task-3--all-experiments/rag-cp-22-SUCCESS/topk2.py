...
def run_rq2_experiment(
    ...
    device='cuda',
    training_piis=None,
    baseline_results=None  # NEW: Accept pre-computed baseline
):
    """
    Run RQ2 experiment: Compare baseline vs RAG exposure
    Uses pre-computed baseline from DEA experiment
    """
    results = {
        'baseline': {},
        'rag': {}
    ...
    for canary_type in canary_types:
        ...
        if baseline_results and canary_type in baseline_results:
            ...
            logger.info(f"  Baseline (from DEA): {baseline_results[canary_type]['mean']:.2f} ± {baseline_results[canary_type]['std']:.2f}")
        else:
            logger.info("  Computing NEW baseline (DEA results not found)")
            baseline_exposures = []

            for i in tqdm(range(num_canaries), desc=f"{canary_type} baseline"):
                canary, secret = generator.generate_canary(canary_type)
        ...
        # Always compute RAG exposures
        logger.info("  Computing RAG exposures...")
        rag_exposures = []
        ...
        logger.info(f"  Reduction: {baseline_mean - rag_mean:.2f}")
